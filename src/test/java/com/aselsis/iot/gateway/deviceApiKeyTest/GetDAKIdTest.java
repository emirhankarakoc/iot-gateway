...
public class GetDAKIdTest {
    ...
    public void getUserId200Test(){
        ...
        ValidatableResponse then= RestAssured.given().baseUri("http://localhost:8080")
                ...
                .get(pathDeviceApiKey+"/" + dak.getId() + "")
                .then();
        then.statusCode(200);
    }
    @Test
    ...
    public void getDAKId404Test(){
        ...
        ValidatableResponse then= RestAssured.given().baseUri("http://localhost:8080")
                .header("Authorization", "Bearer "+token)
                .contentType(ContentType.JSON).body("").get(pathDeviceApiKey+"/"+dak.getId()).then();
        then.statusCode(404);
    }
